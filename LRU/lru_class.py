...
class LRU(nn.Module):
    def __init__(self, in_features, batch_size, activation=torch.softmax, r_min=0.9, r_max=0.999, use_bias=True,
                 unroll=False):
    ...
    def forward(self, inputs):
        ...
        if self.activation is not None:
            output = self.activation(output, dim=0)
